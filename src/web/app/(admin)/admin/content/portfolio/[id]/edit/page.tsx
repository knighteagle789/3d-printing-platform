...
import { z } from 'zod';
import { contentApi, type UpdatePortfolioItemRequest } from '@/lib/api/content';
import { JetBrains_Mono } from 'next/font/google';
import { ArrowLeft, Eye, EyeOff, Star, CheckCircle2, AlertCircle } from 'lucide-react';
import {
...
interface RawItem {
  title: string; description: string; detailedDescription: string | null;
  category: string; imageUrl: string; modelFileUrl: string | null;
  timelapseVideoUrl: string | null; tags: string[] | null;
  projectDetails: string | null; displayOrder: number;
...
export default function EditPortfolioItemPage({
  ...
  useEffect(() => {
    if (data?.data) {
      ...
      setDetailPairs(parseProjectDetails(item.projectDetails));
    }
  }, [data, reset]);
  ...
  const mutation = useMutation({
    ...
    onError: () => showToast('error', 'Failed to save portfolio item.'),
  });
  ...
  if (isLoading) {
    return (
      <div className="space-y-4 max-w-2xl">
        ...
        {[...Array(4)].map((_, i) => <div key={i} className="h-16 bg-white/[0.02] animate-pulse" />)}
      </div>
    );
  }

  const raw = data?.data as RawItem | undefined;
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0 pt-1">
          {raw?.isFeatured && (
            <span className={`${mono.className} inline-flex items-center gap-1 text-[8px] uppercase tracking-[0.12em] text-amber-400`}>
              <Star className="h-2.5 w-2.5" /> Featured
            </span>
          )}
          <span className={`${mono.className} inline-flex items-center gap-1.5 text-[8px] uppercase tracking-[0.15em] px-3 py-1.5 border ${
            raw?.isPublished
              ? 'bg-emerald-400/10 text-emerald-400 border-emerald-400/20'
              : 'bg-white/[0.04] text-white/20 border-white/8'
          }`}>
            {raw?.isPublished ? <Eye className="h-2.5 w-2.5" /> : <EyeOff className="h-2.5 w-2.5" />}
            {raw?.isPublished ? 'Published' : 'Draft'}
          </span>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <FormRow label="Category" error={errors.category?.message}>
            <select key={watch('category')} className={selectCls} {...register('category')}>
        ...
        <FormRow label="Short Description" hint="Shown in the gallery grid" error={errors.description?.message}>
          <textarea rows={3} className={textareaCls} {...register('description')} />
        </FormRow>

        <FormRow label="Detailed Description" optional hint="Shown on the item detail page">
          <textarea rows={5} className={textareaCls} {...register('detailedDescription')} />
        </FormRow>

        <FormRow label="Tags" optional hint="Comma-separated">
          <input type="text" className={inputCls} {...register('tags')} />
        </FormRow>
