  ...
  return (
    <div className="pt-16 bg-page">
      ...
      {!isLoading && allTags.length > 0 && (
        <div className="border-b border-border px-6 py-4 overflow-x-auto bg-surface">
          <div className="max-w-7xl mx-auto flex items-center gap-2 flex-nowrap min-w-0">
            <span className={`${mono.className} text-[9px] uppercase tracking-[0.22em] text-text-muted shrink-0 mr-2`}>
              Filter
            </span>
            <button
          ...
          {!activeTag && totalPages > 1 && (
            <div className={`${mono.className} flex items-center justify-center gap-6 mt-16`}>
              <button
                ...
                className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-[0.2em] text-text-muted hover:text-text-primary disabled:opacity-25 disabled:cursor-not-allowed transition-colors"
              >
      ...
      <section className="relative overflow-hidden bg-accent">
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-28 flex flex-col md:flex-row items-start md:items-end justify-between gap-10">
          <div>
            ...
            >
              TURN YOUR<br />DESIGN INTO<br />REALITY<span className="text-white/40">.</span>
            </h2>
          </div>
...
function FeaturedCard({ post, monoClass }: { post: BlogPostSummary; monoClass: string }) {
  return (
    <Link href={`/blog/${post.slug}`}>
      <article className="group relative border border-border bg-surface hover:bg-surface-alt transition-colors overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-5">
          <div className="lg:col-span-2 aspect-video lg:aspect-auto min-h-[200px] relative bg-surface-alt overflow-hidden">
          ...
          <div className="lg:col-span-3 p-10 flex flex-col justify-between gap-6">
            <div>
              ...
              <h2
                className="font-black tracking-tight leading-[1.1] mb-4 text-text-primary group-hover:text-accent transition-colors"
...
function PostCard({ post, monoClass }: { post: BlogPostSummary; monoClass: string }) {
  return (
    <Link href={`/blog/${post.slug}`}>
      <article className="group bg-surface hover:bg-surface-alt transition-colors h-full flex flex-col p-8">
        {post.tags?.length > 0 && (
        ...
        <h2
          className="font-black tracking-tight leading-[1.15] mb-3 flex-1 text-text-primary group-hover:text-accent transition-colors"
...
function Meta({ post, showArrow, monoClass }: { post: BlogPostSummary; showArrow?: boolean; monoClass: string }) {
  return (
    <div className="flex items-center justify-between pt-4 border-t border-border">
      <div className={`${monoClass} flex items-center gap-4 text-[9px] uppercase tracking-[0.18em] text-text-muted`}>
...
function TagPill({ tag, monoClass }: { tag: string; monoClass: string }) {
  return (
    <span className={`${monoClass} text-[8px] uppercase tracking-[0.18em] border border-accent/25 text-accent/70 px-2 py-0.5`}>
      {tag}
    </span>
  );
}
...
function FeaturedBadge({ monoClass }: { monoClass: string }) {
  return (
    <span className={`${monoClass} text-[8px] uppercase tracking-[0.18em] bg-accent text-white px-2 py-0.5`}>
      Latest
    </span>
  );
}
...
function FeaturedPlaceholder({ title }: { title: string }) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center"
      style={{ background: `hsl(${hue}, 15%, 94%)` }}>
      <div
...
function LoadingSkeleton() {
  return (
    <div className="space-y-px">
      <div className="animate-pulse bg-surface-alt h-72 w-full border border-border" />
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-border">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="animate-pulse bg-surface h-52" />
        ))}
      </div>
    </div>
  );
...
function EmptyState({ monoClass, filtered }: { monoClass: string; filtered?: boolean }) {
  return (
    <div className="py-32 text-center border border-border">
      <p className={`${monoClass} text-[10px] uppercase tracking-[0.25em] text-text-muted`}>
